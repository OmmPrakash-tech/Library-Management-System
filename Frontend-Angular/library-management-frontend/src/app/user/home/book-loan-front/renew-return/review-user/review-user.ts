...
editReviewId!: number;
  ...
  ngOnInit(): void {
    ...
    this.http.get<boolean>(`http://localhost:5050/api/reviews/book/${this.bookId}/exists`)
      .subscribe(res => {
  ...
  submitReview() {
    ...
    this.http.post('http://localhost:5050/api/reviews', this.review)
      .subscribe({
        next: () => {
  ...
  // ✅ GET MY REVIEWS
  getMyReviews() {
    this.http.get<any>('http://localhost:5050/api/reviews/my')
      .subscribe({
        next: (res) => {
...
//   verticalPosition: 'top'        // 👉 top popup
  ...
  this.http.put(`http://localhost:5050/api/reviews/${this.editReviewId}`, this.review)
    .subscribe({
      next: () => {
