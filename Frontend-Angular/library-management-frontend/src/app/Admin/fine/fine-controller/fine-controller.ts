...
types = ['OVERDUE', 'DAMAGE', 'LOSS'];
  ...
  // 🔹 Load fines
  loadFines() {
    this.http.get<any>('https://library-backend-docker.onrender.com/api/fines', {
...
createFine() {
  ...
  this.http.post('https://library-backend-docker.onrender.com/api/fines', payload, {
    headers: this.getAuthHeaders()
  }).subscribe({
    next: () => {
      this.loadFines();
  ...
  waiveFine(id: number) {
    ...
    this.http.post(`https://library-backend-docker.onrender.com/api/fines/${id}/waive`, payload, {
      headers: this.getAuthHeaders()
    }).subscribe({
      next: () => this.loadFines(),
